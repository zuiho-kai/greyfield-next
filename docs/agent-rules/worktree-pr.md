...
- When more than one task is active, the coordinator must maintain a task ledger with issue number, worktree, branch, worker id/name, current status, PR URL if any, and merge state. Use exact lifecycle terms: `in progress`, `patched`, `validated`, `PR opened`, `merged`, or `blocked`.
- Do not answer "the previous task is done" from memory. Check the ledger or live state first: worktree status, commit, pushed branch, PR, and issue/merge state.
...
- Worker assignment is not interchangeable across issues. If a worker has started an issue, keep it on that issue through handoff/review/merge. To start another independent issue, spawn another worker in a separate worktree. Interrupting and repurposing a worker is allowed only when the coordinator explicitly abandons or retires the first task and records that state.
- Sub-agents do not communicate with each other. Any dependency, conflict, status summary, or merge ordering between workers must go through the coordinator.
...
- Before editing a PR in response to "CI is failing", "review comments", or "why is it still red", take one current-head snapshot that includes `headRefOid`, `gh pr checks`, GraphQL `reviewThreads` resolution/outdated state, and the latest bot review body. Classify each signal as `failed`, `pending`, `unresolved review`, `outdated/resolved review`, or `stale review body` before choosing the next fix.
- If the latest bot review body mentions code that no longer exists on the current head, do not refactor to satisfy that stale text. Verify against the current diff and thread state; fix only still-valid findings, and report skipped stale findings briefly.
...
- Desktop launch and recovery work must prove the actual user-visible desktop state. Ports, PIDs, generated URLs, and DOM selectors are diagnostics only. The worker or coordinator must report window bounds on a visible display plus a screenshot/harness artifact or pixel check showing the pet/control/chat surface that the user would see.
...
- Before marking a PR ready, the coordinator must finish a local first-pass review of the worker handoff. CodeRabbit and CI are final gates, not substitute reviewers. This review must include the module-owner edge cases for the touched path; otherwise remote review feedback can force another full CI cycle.
- If a bot review thread becomes outdated or resolved only after the bot-review gate already failed, confirm the current GraphQL thread state. If every bot-authored thread is resolved or explicitly not applicable, rerun the failed bot-review gate instead of pushing a no-op commit.
- When multiple PR signals appear in sequence, do not treat each one as a separate surprise. Re-read the whole live PR state after every push that is meant to unblock CI or review: current head SHA, failed or pending checks, unresolved bot threads, and whether CodeRabbit's latest review body is actionable on the new head.
...
- Before asking for CodeRabbit or merge-readiness CI, the coordinator must complete a local owner review of the pushed head. For provider/settings/runtime changes, this includes missing config, incomplete config, provider request or stream failure, proactive/background error containment, and unrelated settings-field mutation.
