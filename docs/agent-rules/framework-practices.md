...
| #152 V2.3 corrective | Even after #158 was corrected to Screen awareness, the worker and coordinator almost let old screenshot-management vocabulary leak back through Shot/Clear/End controls and long silent implementation loops. | When product feedback removes visible controls, update issue acceptance and harness assertions immediately. For screen awareness, the small panel defaults to one toggle only; sampling policy belongs in Settings/internal defaults. Coordinators must interrupt for status if no validation process, PR, or concrete worker update appears within 10-15 minutes. |
| #176 | The V2.3 vision-routing PR took an extra review/CI cycle because the first coordinator review caught only missing/incomplete Vision config, while CodeRabbit later caught stream-failure propagation and a Settings cross-field mutation. The bot-review gate also failed once on an already-addressed outdated thread before CodeRabbit marked it resolved. | Before marking a PR ready, run the local module-owner checklist for missing config, incomplete config, provider stream/request failure, proactive/background error containment, and unrelated Settings field mutation. Treat CodeRabbit as a final gate, not the first reviewer. When bot-review gate races with outdated or resolved threads, confirm current reviewThreads state and rerun the failed gate instead of pushing churn. |
| #178 | The task-model routing PR implemented the model slots and passed harnesses, but the coordinator accepted a screenshot taken after the harness forcibly scrolled to Provider. A normal user clicked the existing `Model` entry and saw the old Live2D model settings, so the new task-model service felt invisible. | For Settings/navigation/entry-point work, acceptance must prove the first-glance user path: default Settings surface, visible nav label, click target, active section, and screenshot after the click. Do not treat DOM existence or `scrollIntoView` evidence as discoverability. If an old label such as `Model` now conflicts with a new model-service concept, rename or reorder the entry before merging. |
| #215 | The memory-copy PR required several visible passes because CI failure, bot review gate failure, pending split frontend jobs, and stale CodeRabbit review text were inspected one after another instead of as one current-head PR state. | When debugging a red PR or review feedback, first snapshot the live head SHA, all check conclusions, unresolved review threads, and latest bot review body. Classify signals as failed, pending, unresolved, outdated/resolved, or stale before editing so CI fixes and review fixes do not become serial rework. |
