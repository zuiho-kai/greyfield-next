...
The latest V1 acceptance code evidence head `4479c2622ede94337808effbcb65b3c82b35b2ee` includes #41, #42, #43, #44, #45, #46, and #55. Local current-head verification passed on that head:

- `pnpm typecheck` -> passed.
- `pnpm test:backend` -> 26 files / 96 tests passed.
...
- `pnpm harness:frontend-full` -> 16 checks passed in 2m 39s, including the microphone ASR and waveform mouth harness.
- Screenshot artifacts from `.cache/greyfield-v1-visual-acceptance/latest/` were inspected after the current-head run: Settings had no horizontal overflow, Chat controls fit, and the Pet window used Hiyori with an in-viewport bubble.

The most recent GitHub Actions checkpoint retained in this document is run `28072461072` on #46 head `c53b70916ef67543ff80387a2e9af1edaeb26ec3`, which passed Fast checks, Desktop pet quick harness, and `frontend-full`.
...
#55 full-voice closeout added the two missing V1 voice paths on main:

- `pnpm typecheck` -> passed.
- Targeted core/audio/main/renderer/persistence tests -> 9 files / 90 tests passed.
- `pnpm test:backend` -> 26 files / 96 tests passed.
- `pnpm test:frontend` -> 26 files / 108 tests passed.
- `pnpm harness:acceptance` -> fake text and fake audio input paths passed.
- `pnpm harness:electron:voice-input` -> `microphoneStopCanceledInput: true`, `transcriptReachedChat: true`, `ttsPlaybackStarted: true`, `waveformMouthMoved: true`, `stopCanceledPlayback: true`, `queueCleared: true`, `mouthOpenReset: true`.
- `pnpm harness:frontend-full` -> 16 checks passed in 2m 39s on main head `4479c262`, including the new microphone ASR and waveform mouth harness.
...
| Load a real Live2D `.model3.json` without counting fallback as acceptance | `pnpm harness:live2d`; unit tests `model-manifest`, `model3-parser`, `live2d-deps`; main `frontend-full` on `4479c262` | Current on main through #55 |
| Send a text message and receive streamed/final Chat output | `pnpm harness:electron`; `pnpm harness:acceptance`; main `frontend-full` on `4479c262`; `pnpm harness:electron:real-llm` when real provider env is supplied | Fake/failure/abort paths current on main; real provider requires env rerun |
| Stop a running text reply | Main evidence: `pnpm harness:electron:provider-abort`; merged #41 proof that Stop is clickable during streaming, shows Stopped, aborts the provider request, and does not append the old partial reply; main `frontend-full` on `4479c262` | Current on main through #55 |
| Understand Chat state and retry after provider failure | Main evidence: `pnpm harness:electron:provider-failure`; merged #41 Waiting / Generating / Stopped / Failed / Retry-ready UI and retry-button proof; #46 timeout harness stability; main `frontend-full` on `4479c262` | Current on main through #55 |
