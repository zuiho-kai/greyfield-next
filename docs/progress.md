...
| Windows portable preview | PR-local acceptance passed | `pnpm harness:windows:portable` launched the same external copied executable, proved `file:` renderers, non-fallback Live2D, trial/provider/real reply/Stop/restart paths, and produced `.cache/greyfield-packaged-smoke/latest/summary.json` plus six screenshots; accepted artifact is 92,657,677 bytes, SHA-256 `01f6855e77c0c8358fd5ef6a5120e12f1799c263bb270908aff6c74e8ec0e664` | Unsigned and not published; SmartScreen, no updater, built-in model only, and no custom absolute-model-path acceptance remain explicit boundaries. |
...
- Issue #223 PR-local artifact acceptance passed for `Greyfield-0.1.0-preview.1-win-x64-portable.exe` (92,657,677 bytes; SHA-256 `01f6855e77c0c8358fd5ef6a5120e12f1799c263bb270908aff6c74e8ec0e664`). The packaged smoke copied that same executable outside the repository and proved visible Pet/Controls windows, `file:` renderer URLs, built-in Live2D `usedFallback=false`, trial and real OpenAI-compatible paths, provider/audio/mouth Stop, userData-only writes, unchanged repository hashes, and recent-context continuity after relaunch.
...
- V2.1f defines the current memory benchmark gate: `pnpm harness:memory-benchmark` is fixture-driven and now covers long-chat summary traceability, deterministic atom extraction, atom recall, semantic/relationship recall, source visibility, prompt material without atom ID/database terminology, disabled/noise rejection, false-positive rejection, prompt budget skips, generic episodic scene extraction, and pure core rainy virtual-home hotpot proactive candidates with low-disturbance negative cases for recent activity, ordinary weather, missing virtual-home state, and non-shared scenes. Summary and current-role atom Memory Library controls are covered by `pnpm harness:electron:memory-control`, `pnpm harness:electron:memory-atom-library`, and `pnpm harness:frontend-full`; the memory benchmark remains a non-UI gate and does not dynamically prove the UI. CI fails below the recorded fixture baselines and uploads `.cache/greyfield-memory-benchmark/latest/summary.json`.
...
- Remaining V2.1 gaps are still explicit: no renderer Memory Library source-drilldown UI, no production scheduler that gathers real desktop/virtual-home scene signals, no external weather or virtual-home state feed, no Settings/desktop UX for enabling LLM atom extraction by default, broader privacy classification is still limited, and atom clear still does not erase raw turns or summaries.
- Product-planning retro: V2.1 memory status was being communicated as PR/benchmark accounting before the user-visible product story. `AGENTS.md`, `docs/agent-rules/worktree-pr.md`, `docs/agent-rules/framework-practices.md`, `docs/qa-retro.md`, `docs/plans/version-product-book.md`, and `docs/plans/v2-memory-goal.md` now require product-language-first planning: user-feelable now, backend/benchmark-only now, missing gaps, then the next atomic slice.

## 2026-06-29

- V2.1 #125 MaiBot parity closeout was rerun on current-head `73e275b44e0d04191af0fb874f96d722229a4e04`, after #118, #121, #123, and #135 were merged and closed. This closeout updates docs plus the benchmark fixture baselines; it does not add new runtime, UI, provider, or scheduler behavior.
- Current-head evidence passed: `pnpm harness:memory-benchmark`; `pnpm harness:electron:memory-control`; `pnpm harness:electron:memory-atom-library`; `pnpm harness:electron:proactive-desktop-message`; `pnpm harness:frontend-full`.
- `pnpm harness:memory-benchmark` passed with locked baselines `summaryRegressionScore: 1`, `recallRegressionScore: 1`, `atomExtractionScore: 0.95`, `atomRecallScore: 0.99`, `atomWritebackScore: 1`, `proactiveTriggerScore: 1`, `productReadinessScore: 0.581`, and `v21aScenarioScore: 0.573`; computed scores were `summaryRegressionScore: 1`, `recallRegressionScore: 1`, `atomExtractionScore: 1`, `atomRecallScore: 0.995`, `atomWritebackScore: 1`, `proactiveTriggerScore: 1`, `productReadinessScore: 0.581`, `productReadinessCapabilityScore: 0.585`, and `v21aScenarioScore: 0.573`.
- `pnpm harness:electron:memory-control` proved `deletedMemoryErasedRawSourceEvidence`, `memoryExportExcludedProviderSecret`, and `summaryIncludesSourceTurns`. `pnpm harness:electron:memory-atom-library` proved vague opinion, promise, relationship-date, and scene recall paths, false-positive rejection, user-facing recall text without internal IDs, atom delete removal from prompt/export, and provider-secret export exclusion. `pnpm harness:electron:proactive-desktop-message` proved a natural desktop bubble plus quiet-window, recent-activity, missing-signal, cooldown, global-disable, and chat-history isolation gates.
- `pnpm harness:frontend-full` passed 19 checks in 6m05s, including memory-control, memory-atom-library, proactive desktop message, and restart-context coverage; real OpenAI-compatible TTS remained skipped because no real TTS environment was configured.
- Visual artifacts inspected: `.cache/greyfield-memory-control/latest/settings-memory-source-passages.png`, `.cache/greyfield-memory-atom-library/latest/source-available.png`, and `.cache/greyfield-proactive-desktop-message/latest/proactive-memory-bubble.png`. The source drawers and proactive bubble were visible, readable, and not overlapping core UI.
- Minimum MaiBot-style long-chat memory loop conclusion: V2.1 is sufficient to close the minimum loop, not full memory-product parity. Old chat can leave short context as source-linked summaries; structured atoms can be extracted, written automatically, deduped, recalled, and source-linked for representative facts, preferences, opinions, dates, promises, relationship rituals, and scenes; Settings Memory can manage summary/current-role atom memory; deleted remembered evidence is hidden from product recall/source/export/prompt paths; and a low-disturbance proactive memory bubble can be displayed on the desktop path. A-Memorix-class memory infrastructure is now V3 backlog: full embedding/vector DB recall, node-edge relationship graph, graph/vector retrieval fusion, backfill/migration, and graph/source/profile management. Other post-V2.1 product work still includes real external scene feeds, polished source-explanation UI for every memory type, default Settings/desktop LLM-extraction UX, and broader privacy classification.
- #125 parity gates: long-chat compression is done; structured long-term memory is partial but benchmarked through scripted/hybrid atom extraction, automatic writeback, deterministic fallback, dedupe, and source evidence; trigger recall is partial because exact/alias/secondary/calendar/deterministic relationship/core scene/proactive desktop paths are covered while full vector/relationship graph and real environment feeds are not; original-text drilldown is partial because prompt material and harnessed Memory Library source views exist but complete source-explanation UX is future work; manageable/degradable memory is partial because Memory Library controls, role isolation, no-pending-approval UX, no-vector fallback, provider-secret exclusion, and deleted-evidence filtering are proved while raw JSONL local non-product storage and broader privacy classification remain post-V2.1. Full vector DB and node-edge relationship graph are explicitly V3 backlog, not V2.2 scope.
