...
| Persona/recent context | Core prompt assembly done; desktop persistence in progress | `GFN-V1-007` now tracks package-level prompt assembly only; `GFN-V1-015` tracks Electron persona/memory/JSONL session/restart continuity separately | Main runtime still uses in-memory session and fake memory until Phase E lands. |
| Renderer/harness complexity | Improved in PR #1 | Largest hotspots reduced: `App.vue` 278 lines, `desktop-runtime-bridge.ts` 261, `electron-check.ts` 304 plus `electron-check-helpers.ts` 232 | `Live2DStageView.vue` remains a future split target; harness can later split by pet/settings/chat files. |
