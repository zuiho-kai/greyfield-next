...
| Settings/chat shell | Done on main, closeout polish active | Full Electron harness opens settings/chat and verifies isolation; Chat renders Waiting/Generating/Stopped/Failed/Retry-ready states; Stop remains available during streaming and queued voice playback; provider readiness panel shows Preview/blocked/ready-to-test; Test LLM success/failure/testing/active-chat rejection are covered by harnesses; `frontend-full` now protects these flows on frontend-visible PRs | Settings visual polish and destroyed-window menu recovery are V1 closeout quality fixes, not V1-after feature work. |
