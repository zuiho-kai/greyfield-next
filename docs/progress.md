...
| TTS output | Current local closeout done; needs merge/current-head rerun | Settings exposes `Test Voice`; `pnpm harness:real-tts` proves the configured OpenAI-compatible TTS endpoint returns playable MP3 bytes; `pnpm harness:electron:real-tts` proves Settings `Test Voice`, real TTS bytes entering Electron renderer playback, natural playback queue cleanup, Stop audio cancel, and mouth-open reset; `frontend-full` runs the real TTS Electron harness when credentials are supplied | ASR/microphone input and true decoded-audio energy mouth tracking remain post-V1. |
