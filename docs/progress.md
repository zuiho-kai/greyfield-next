...
| Voice input/output | Done on main | Settings exposes `Test Voice`; `pnpm harness:real-tts` proves the configured OpenAI-compatible TTS endpoint returns playable MP3 bytes when credentials are supplied; `pnpm harness:electron:real-tts` proves Settings `Test Voice`, real TTS bytes entering Electron renderer playback, natural playback queue cleanup, Stop audio cancel, and mouth-open reset; #55 adds browser microphone recording, OpenAI-compatible `/audio/transcriptions` ASR, transcript-to-chat routing, decoded PCM mouth-open playback, and `pnpm harness:electron:voice-input`; main head `4479c262` passed `pnpm typecheck`, `pnpm test:backend`, `pnpm test:frontend`, `pnpm harness:acceptance`, `pnpm harness:electron:voice-input`, and `pnpm harness:frontend-full` with 16 checks in 2m 39s | Real external LLM/TTS endpoint demos still require env-backed reruns before a credentialed release demo claim. |
