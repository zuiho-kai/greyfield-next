...
- Tightened provider failure handling: core runtime now writes user/assistant turns only after a non-interrupted final reply, so provider failure or interrupt does not leave half turns in recent context. Added `pnpm harness:electron:provider-failure` for 401, 404, and malformed SSE chat failures; each shows a readable error, restores the draft, and keeps JSONL session clean.
