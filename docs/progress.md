...
- V2.1 #114 MaiBot parity closeout was rerun on current-head `1997fd2d1128fdb697ed64f12a3dc7d1be6ddae8`, the merge commit for #116. This is a docs/evidence-only closeout; no runtime, UI, provider, harness, or `packages/dev-harness/v1-features.json` behavior changed.
- Current-head evidence passed: `pnpm typecheck`; `pnpm harness:memory-benchmark`; `pnpm harness:electron:memory-control`; `pnpm harness:electron:memory-atom-library`; `pnpm harness:electron:proactive-desktop-message`.
- `pnpm harness:memory-benchmark` fixture version 6 passed with locked baselines `summaryRegressionScore: 1`, `recallRegressionScore: 1`, `atomExtractionScore: 0.95`, `atomRecallScore: 0.8`, `proactiveTriggerScore: 1`, `productReadinessScore: 0.56`, and `v21aScenarioScore: 0.54`; computed scores were `summaryRegressionScore: 1`, `recallRegressionScore: 1`, `atomExtractionScore: 1`, `atomRecallScore: 0.86`, `proactiveTriggerScore: 1`, `productReadinessScore: 0.567`, `productReadinessCapabilityScore: 0.574`, and `v21aScenarioScore: 0.551`.
- PR-local #120 evidence updates `pnpm harness:memory-benchmark` to fixture version 7 with a deterministic semantic/relationship graph-equivalent recall case: vague "important day / gift ritual" input recalls the first-meeting rose relationship atom with source turn and raw source fragments while colleague/wedding gift prompts are rejected. Locked baselines are `summaryRegressionScore: 1`, `recallRegressionScore: 1`, `atomExtractionScore: 0.95`, `atomRecallScore: 0.88`, `proactiveTriggerScore: 1`, `productReadinessScore: 0.58`, and `v21aScenarioScore: 0.56`; computed scores were `summaryRegressionScore: 1`, `recallRegressionScore: 1`, `atomExtractionScore: 1`, `atomRecallScore: 0.883`, `proactiveTriggerScore: 1`, `productReadinessScore: 0.582`, `productReadinessCapabilityScore: 0.591`, and `v21aScenarioScore: 0.563`.
- Minimum MaiBot-style long-chat memory loop conclusion: V2.1 is sufficient to close the minimum loop, not full memory-product parity. Old chat can leave short context as source-linked summaries; structured atoms can be extracted and recalled for representative facts, preferences, opinions, dates, relationship rituals, and scenes; recall can inject bounded raw source fragments; Settings Memory can manage summary/current-role atom memory; and a proactive memory bubble can be displayed on the desktop path. The remaining low product-readiness score keeps full embedding/vector DB recall, renderer source-drilldown UI, production environment feeds/scheduler, default LLM-extraction UX, and broader privacy/erasure semantics post-V2.1.
- #114 parity gates: long-chat compression is done; structured long-term memory is partial because scripted/hybrid atom extraction and deterministic fallback exist but default desktop LLM extraction and broader privacy are still unfinished; trigger recall is partial because exact/alias/secondary/calendar/core scene candidates and the #120 deterministic semantic/relationship minimum path are covered, while a full embedding/vector DB and production environment feeds are not; original-text drilldown is partial because prompt material can include source fragments but renderer drilldown UI is still missing; manageable/degradable memory is partial because Memory Library controls, role isolation, no-pending-approval UX, and no-vector fallback are proved, while atom clear still keeps raw turns/summaries and full erasure semantics are post-V2.1.
- Issue-status recommendation after the #114 PR merges: close #75, #76, #77, and #79 for the V2.1 minimum slice, while moving their broader semantic/vector/relationship graph, real scene-feed scheduler, renderer source-drilldown UI, default LLM extraction UX, and stronger privacy/erasure work to V2.2+ issues. Do not bulk-close parent issues before the closeout PR documents this handoff.
