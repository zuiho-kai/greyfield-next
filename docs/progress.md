...
| Settings/chat shell | Done on main; #59 has PR-local controls/UI closeout | Full Electron harness opens settings/chat and verifies isolation; Chat renders Waiting/Generating/Stopped/Failed/Retry-ready states; Stop remains available during streaming and queued voice playback; provider readiness panel shows Preview/blocked/ready-to-test; Test LLM success/failure/testing/active-chat rejection are covered by harnesses; #45 fixed Settings visual polish and destroyed-window menu recovery; #59 adds a separate draggable floating controls window and fixes narrow Settings scale/position controls in PR-local `frontend-full` evidence | After #59 merges, rerun `pnpm harness:frontend-full` on `main` before final V1 release wording. Continue visual/lifecycle review when Settings, Chat, controls, or Electron window lifecycle changes. |
| Voice input/output | Done on main; #59 fixes PR-local speech playback overlap | Settings exposes `Test Voice`; `pnpm harness:real-tts` proves the configured OpenAI-compatible TTS endpoint returns playable MP3 bytes when credentials are supplied; `pnpm harness:electron:real-tts` proves Settings `Test Voice`, real TTS bytes entering Electron renderer playback, natural playback queue cleanup, Stop audio cancel, and mouth-open reset; #55 adds browser microphone recording, OpenAI-compatible `/audio/transcriptions` ASR, transcript-to-chat routing, decoded PCM mouth-open playback, and `pnpm harness:electron:voice-input`; main head `4479c262` passed `pnpm typecheck`, `pnpm test:backend`, `pnpm test:frontend`, `pnpm harness:acceptance`, `pnpm harness:electron:voice-input`, and `pnpm harness:frontend-full` with 16 checks in 2m 39s; #59 PR-local `pnpm harness:electron:stop-audio` reports `noOverlappingSpeech: true` | Real external LLM/TTS endpoint demos still require env-backed reruns before a credentialed release demo claim. After #59 merges, rerun main `frontend-full` so non-overlap evidence becomes release evidence. |
...
- #59 PR-local evidence: `pnpm typecheck`, targeted audio/runtime/bubble/visual tests, `pnpm harness:electron`, `pnpm harness:electron:stop-audio`, `pnpm harness:electron:voice-input`, and `pnpm harness:frontend-full` all passed. The latest `frontend-full` passed 16 checks in 3m 45s; optional real OpenAI-compatible TTS Electron proof was skipped because credential env vars were absent.
...
- Remaining V1 release action: merge #59, then rerun current-head `pnpm harness:frontend-full` on `main` and update `docs/v1-completion-evidence.md` from that merged evidence.
