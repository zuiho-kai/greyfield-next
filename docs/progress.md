...
- Minimum MaiBot-style long-chat memory loop conclusion: V2.1 is sufficient to close the minimum loop, not full memory-product parity. Old chat can leave short context as source-linked summaries; structured atoms can be extracted and recalled for representative facts, preferences, opinions, dates, and scenes; recall can inject bounded raw source fragments; Settings Memory can manage summary/current-role atom memory; and a proactive memory bubble can be displayed on the desktop path. The remaining low product-readiness score keeps broader semantic/vector recall, full source-drilldown UI, production environment feeds/scheduler, default LLM-extraction UX, and broader privacy/erasure semantics post-V2.1.
- #114 parity gates: long-chat compression is done; structured long-term memory is partial because scripted/hybrid atom extraction and deterministic fallback exist but default desktop LLM extraction and broader privacy are still unfinished; trigger recall is partial because exact/alias/secondary/calendar/core scene candidates are covered but broad semantic/vector, relationship graph, and real environment feeds are post-V2.1; original-text drilldown is partial because prompt material can include source fragments but renderer drilldown UI is still missing; manageable/degradable memory is partial because Memory Library controls, role isolation, no-pending-approval UX, and no-vector fallback are proved, while atom clear still keeps raw turns/summaries and full erasure semantics are post-V2.1.
