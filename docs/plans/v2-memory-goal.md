...
- `frontend-full-check.ts` runs `pnpm harness:electron:memory-control` and `pnpm harness:electron:memory-atom-library`, so frontend-visible memory regressions are covered by the aggregate gate.
