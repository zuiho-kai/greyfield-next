...
const enUS: Record<SettingsI18nKey, string> = {
  ...
  "memory.standard.detail": "Basic memory saves explicit facts from local chat, such as names, dates, and preferences. On later turns, Greyfield searches the memory library for relevant items and inserts the matching text into the prompt context. It does not call an extra memory model.",
  ...
  "memory.ready.detail": "Enhanced memory adds one step before saving: Greyfield calls the configured memory model to extract subtler long-term details such as relationships, promises, scenes, and opinions. Recall is still controlled by the memory library and inserted into later prompt context; this mode costs API tokens.",
  ...
  "memory.needsBaseUrl": "Enhanced memory needs a chat provider Base URL. Basic local memory stays on until the provider is ready.",
  ...
  "memory.about.detail": "Both modes end up in the same place: relevant memory text is recalled from the library and added to the next chat prompt so Greyfield can answer with context. Basic memory only saves clear local facts. Enhanced memory also asks the memory model to extract hidden preferences, relationships, promises, scenes, and opinions before saving.",
...
const zhCN: Partial<Record<SettingsI18nKey, string>> = {
  ...
  "memory.standard.detail": "基础记忆会从本地聊天里保存明确事实，比如名字、日期、偏好。之后聊天时，Greyfield 会从记忆库里检索相关条目，把匹配到的记忆文本塞进本轮提示词上下文；这个过程不额外调用记忆模型。",
  ...
  "memory.ready.detail": "增强记忆会在保存前多走一步：调用你配置的记忆模型，抽取关系、承诺、场景、观点等更隐含的长期信息。召回时仍然从记忆库取相关文本并塞进后续提示词上下文；这个模式会消耗 API token。",
  "memory.needsProvider": "增强记忆需要 OpenAI 兼容聊天服务。在服务就绪前，基础本地记忆会继续开启。",
  "memory.needsBaseUrl": "增强记忆需要聊天服务 Base URL。在服务就绪前，基础本地记忆会继续开启。",
  ...
  "memory.about.detail": "两种模式最后都会走同一条路径：从记忆库召回相关记忆文本，加入下一轮聊天提示词，让 Greyfield 带着上下文回答。基础记忆只保存明确事实；增强记忆会先让记忆模型理解隐含偏好、关系、承诺、场景和观点，再保存。",
