...
const enUS: Record<SettingsI18nKey, string> = {
  ...
  "memory.standard.detail": "Basic memory is local and lightweight. Greyfield can still remember simple facts, names, dates, and preferences without calling an LLM.",
  ...
  "memory.ready.detail": "Enhanced memory is on. Greyfield will call the configured memory model to understand richer long-term details such as preferences, relationships, promises, scenes, and opinions. If that call fails, basic local memory still works.",
  ...
  "memory.about.title": "Memory modes",
  "memory.about.detail": "Basic memory is always the safe local fallback: simple facts, names, dates, and preferences. Enhanced memory uses your configured memory model to understand richer context, so it can cost API tokens but remembers more subtle details.",
...
const zhCN: Partial<Record<SettingsI18nKey, string>> = {
  ...
  "memory.standard.detail": "基础记忆是本地轻量模式。即使不开增强记忆，Greyfield 也会记住名字、日期、偏好等简单事实，而且不调用模型。",
  ...
  "memory.ready.detail": "增强记忆已开启。Greyfield 会调用你配置的记忆模型，理解偏好、关系、承诺、场景、观点等更细的长期信息；如果调用失败，会自动退回基础本地记忆。",
  ...
  "memory.about.title": "记忆模式说明",
  "memory.about.detail": "基础记忆是安全的本地兜底：记名字、日期、偏好等简单事实。增强记忆会使用你配置的记忆模型理解更复杂的上下文，会消耗 API token，但能记住更细微的信息。",
