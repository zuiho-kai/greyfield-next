...
const enUS: Record<SettingsI18nKey, string> = {
  ...
  "memory.standard.detail": "Basic memory saves explicit facts from local chat, such as names, dates, and preferences. Recall itself does not use a model: before each reply, Greyfield locally scores memory items by cues and keywords, then inserts the matching text into the prompt context.",
  ...
  "memory.about.detail": "Greyfield keeps memory local by default. It writes recent turns into the current character's memory store, recalls matching saved details before replies, and can optionally use the configured Memory model to extract richer long-term facts.",
...
const zhCN: Partial<Record<SettingsI18nKey, string>> = {
  ...
  "memory.standard.detail": "基础记忆会从本地聊天里保存明确事实，比如名字、日期、偏好。召回本身不用模型：每次回复前，Greyfield 会按线索/关键词在记忆库里本地打分，挑出相关记忆文本塞进本轮提示词上下文。",
  ...
  "memory.about.detail": "Greyfield 默认把记忆保存在本地。它会把当前角色的对话写入记忆库，回复前召回匹配的已保存细节；也可以按需使用你配置的“记忆模型”抽取更丰富的长期事实。",
