const launchVbs = `Option Explicit
...
const stopVbs = `Option Explicit

Dim shell, fso, root, ps1, command

Set fso = CreateObject("Scripting.FileSystemObject")
root = fso.GetParentFolderName(WScript.ScriptFullName)
ps1 = fso.BuildPath(fso.BuildPath(root, "scripts"), "stop-greyfield-windows.ps1")

If Not fso.FileExists(ps1) Then
  MsgBox "Greyfield Windows stop script was not found:" & vbCrLf & ps1, vbCritical, "Greyfield stop failed"
  WScript.Quit 1
End If
...
export function buildLauncherFiles() {
  return [
    { relativePath: "Launch Greyfield.vbs", content: launchVbs },
    { relativePath: "Stop Greyfield.vbs", content: stopVbs },
    { relativePath: "scripts/launch-greyfield-windows.ps1", content: launchPowerShell },
    { relativePath: "scripts/stop-greyfield-windows.ps1", content: stopPowerShell }
  ];
}
