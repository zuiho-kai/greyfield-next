...
- Small frontend-visible changes must use bounded validation: prove the touched ordinary user path with targeted tests, a focused visual/Electron harness, and inspected artifacts. Do not run the aggregate frontend gate by default for low-risk UI/copy/nav changes unless the issue, coordinator, or merge decision explicitly needs it.
- When fixing stale or contradictory UI copy/state in Settings or another visible section, audit the whole visible section, run a negative text search for the stale user-facing copy across source and relevant harnesses, and update any tests that protect the old product state before handoff.
- Any UI where the user can create or add persistent data must be reviewed as a full lifecycle surface: create, display, update or disable if applicable, restore if disabled, and true delete. Hide/disable is not delete. If true deletion is intentionally unavailable, the PR must state the product reason and the alternate user escape path.
- User-data management surfaces such as memory, profile, sources, credentials, and temporary captures must keep soft-delete wording and hard-delete behavior separate across UI labels, IPC names, runtime methods, store methods, and tests.
- If an aggregate frontend gate fails outside the PR's changed user path, stop and report it as an unrelated gate blocker with command, error, and already-passing targeted evidence. Do not modify unrelated runtime or harness behavior in the same PR without coordinator approval.
- Navigation, Settings, onboarding, or entry-point PRs must prove first-glance discoverability: open the ordinary surface from a fresh/default state, show the entry is visible without hidden scrolling or developer-only shortcuts, click it, and inspect the resulting screenshot. DOM existence or a harness `scrollIntoView` screenshot is not user acceptance.
- Settings and other narrow-panel form changes must validate local control geometry in the actual container width. DOM text, route existence, or document-level overflow checks are not enough; inspect a screenshot or bounding boxes for the touched subsection and confirm select/input/button text does not collapse, overlap, or truncate.
...
- Before marking a PR ready for CodeRabbit or CI, the coordinator must run a real first-pass review locally. CodeRabbit is a merge-readiness gate, not the first reviewer; do not rely on it to catch module-owner edge cases.
- Provider/settings/runtime PRs must cover missing config, invalid or incomplete config, and provider request/stream failure before ready. Background and proactive paths must degrade without unhandled exceptions, and settings edits must not mutate unrelated provider or task fields.
...
- For parallel implementation, one independent issue gets one worker, one worktree, one branch, and one expected PR. Sub-agents do not coordinate with each other; the main coordinator owns the task ledger, collision handling, status reporting, review, and merge sequencing.
...
- Do not repurpose an active implementation worker for a different issue unless the coordinator explicitly retires or abandons the original task and records the handoff state. If a second independent issue appears, spawn a second worker in a separate worktree instead of interrupting the first worker's ownership.
- A worker PR is not finished at first handoff. Keep the worker available through coordinator review, CodeRabbit or human review, CI reruns, and merge; close it only after merge, abandon, or explicit retirement.
...
- Status words must be literal: `patched` means local diff only, `validated` means named local verification passed, `PR opened` means a pushed branch and PR URL exist, and `merged` means the PR is merged and the linked issue close condition is satisfied. Do not call an issue complete from process state, a half-finished local diff, or an unmerged PR.
